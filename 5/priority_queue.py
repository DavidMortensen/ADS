...
class Node:
  """
  The Node class is what we use for our items. We store 4 values in our node.
  :param info: State name
  :param priority: Quota for our state 
  :param seats: The amount of assigned seats
  :param population: The total population for our state
  """
  def __init__(self, info, priority, population, seats):
    self.info = info
    self.priority = priority
    self.seats = seats
    self.population = population
    
# class for a Max Priority Queue
class MaxPQ:
  """
  ...
  def _less(self, i, j):
    ...
    """
    return self._pq[i].priority < self._pq[j].priority
